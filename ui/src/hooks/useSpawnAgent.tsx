...
import { Issue, Project, RepoReadiness, agentInstalled, inspectRepo, rememberedModel, startIssueRun } from "../api";
...
export function useSpawnAgent(project: Project | null, onRepoResolved?: () => void) {
  ...
  async function startIssue(issue: Issue, agentId: string, opts?: SpawnOpts) {
    ...
    const run = await startIssueRun(issue.id, agentId, model, opts?.base, opts?.mergeTarget);
    await refreshRuns();
